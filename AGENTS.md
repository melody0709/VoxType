...
- **【A】`llm::Refine()` 的返回值不是纯文本**：它返回 `{text, rawLlmText, guardRejected}`。`text` 是上屏文本（被 `IsLikelyAssistantReply` 守卫拦下时回退为原文），`rawLlmText` 才是模型原始输出。`enableLlmDebug` 日志必须记 `rawLlmText`，否则守卫命中后无法复盘模型到底说了什么。守卫是**部分网**（只拦应答词开头的回复），完备性依赖提示词的边界声明，不得把它当成唯一防线。
