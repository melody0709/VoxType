...
void TabLlm::CreateControls(HWND parent) {
    m_controls.clear();
    HWND control = CreateLabel(parent, S(UiStyle::ContentLeft), S(UiStyle::RowLabelY(0)), S(UiStyle::LabelWidth), S(UiStyle::LabelH), L"Provider");
    AddLlmControl(control);
    AddLlmControl(CreateCombo(parent, IDC_LLM_PROVIDER, S(UiStyle::InputLeft), S(UiStyle::RowInputY(0)), S(480), S(400)));
    AddLlmControl(CreateButton(parent, IDC_LLM_PROVIDER_ADD, S(UiStyle::SideBtnX), S(UiStyle::RowInputY(0)) - S(1), S(44), S(UiStyle::BtnH), L"+"));
    AddLlmControl(CreateButton(parent, IDC_LLM_PROVIDER_DEL, S(UiStyle::SideBtnX) + S(48), S(UiStyle::RowInputY(0)) - S(1), S(44), S(UiStyle::BtnH), L"\u2212"));

    control = CreateLabel(parent, S(UiStyle::ContentLeft), S(UiStyle::RowLabelY(1)), S(UiStyle::LabelWidth), S(UiStyle::LabelH), L"API Base URL");
    AddLlmControl(control);
    HWND llmEndpoint = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", nullptr, WS_CHILD | WS_VISIBLE | WS_TABSTOP | ES_AUTOHSCROLL,
                                       S(UiStyle::InputLeft), S(UiStyle::RowInputY(1)), S(UiStyle::InputWFull), S(UiStyle::EditH), parent, reinterpret_cast<HMENU>(static_cast<INT_PTR>(IDC_LLM_ENDPOINT)), GetParentInstance(parent), nullptr);
    ...
    HWND llmKey = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", nullptr, WS_CHILD | WS_VISIBLE | WS_TABSTOP | ES_AUTOHSCROLL | ES_PASSWORD,
                                  S(UiStyle::InputLeft), S(UiStyle::RowInputY(2)), S(UiStyle::InputW), S(UiStyle::EditH), parent, reinterpret_cast<HMENU>(static_cast<INT_PTR>(IDC_LLM_KEY)), GetParentInstance(parent), nullptr);
    ApplyUiFont(llmKey);
    AddLlmControl(llmKey);
    AddLlmControl(CreateButton(parent, IDC_LLM_SHOW_KEY, S(UiStyle::SideBtnX), S(UiStyle::RowInputY(2)) - S(1), S(UiStyle::SideBtnW), S(UiStyle::BtnH), L"Show"));
    ...
    HWND llmModel = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", nullptr, WS_CHILD | WS_VISIBLE | WS_TABSTOP | ES_AUTOHSCROLL,
                                    S(UiStyle::InputLeft), S(UiStyle::RowInputY(3)), S(UiStyle::InputWFull), S(UiStyle::EditH), parent, reinterpret_cast<HMENU>(static_cast<INT_PTR>(IDC_LLM_MODEL)), GetParentInstance(parent), nullptr);
    ApplyUiFont(llmModel);
    AddLlmControl(llmModel);

    AddLlmControl(CreateButton(parent, IDC_LLM_TEST, S(UiStyle::InputLeft), S(UiStyle::RowInputY(4)), S(UiStyle::ActionBtnW), S(UiStyle::ActionBtnH), L"Test Connection"));
    HWND llmDebug = CreateWindowW(L"BUTTON", L"Log refine before/after", WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_AUTOCHECKBOX,
                                  S(348), S(UiStyle::RowInputY(4)) + S(6), S(220), S(UiStyle::CheckH), parent, reinterpret_cast<HMENU>(static_cast<INT_PTR>(IDC_LLM_DEBUG)), GetParentInstance(parent), nullptr);
...
bool TabLlm::HandleCommand(HWND parent, WORD notifyCode, WORD controlId, HWND control) {
    ...
    case IDC_LLM_PROVIDER:
        if (notifyCode == CBN_SELCHANGE) {
            ...
            if (!prov.empty()) {
                ...
                HWND delBtn = GetDlgItem(parent, IDC_LLM_PROVIDER_DEL);
                if (delBtn) EnableWindow(delBtn, FindPresetIndex(prov) < 0);
                HWND resetBtn = GetDlgItem(parent, IDC_LLM_EXTRA_RESET);
    ...
    case IDC_LLM_PROVIDER_ADD: {
        ...
        if (ShowInputDialog(parent, L"Add Provider", name)) {
            ...
            if (exists) {
                ...
                EnableWindow(GetDlgItem(parent, IDC_LLM_PROVIDER_DEL), TRUE);
    ...
    case IDC_LLM_PROVIDER_DEL: {
        ...
        SetWindowTextW(GetDlgItem(parent, IDC_LLM_MODEL), g_config.llmModel.c_str());
        SetWindowTextW(GetDlgItem(parent, IDC_LLM_EXTRA), g_config.llmExtraParams.c_str());
        EnableWindow(GetDlgItem(parent, IDC_LLM_PROVIDER_DEL), FALSE);
