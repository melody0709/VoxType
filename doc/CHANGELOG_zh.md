...
- **已保存的提示词会升级，不再静默失效**：预设此前按字符串全等匹配，只要预设文案动一个字，用户配置就会被判为 `Custom`，且**界面显示正常却继续发送旧提示词**。现在由 `llm_prompt_preset` / `llm_prompt_preset_version` 记录提示词来自哪个内置预设，加载时识别 v1 文案并升级为 v2，用户手改过的内容一律固定为 `custom` 且永不覆盖。`Manage...` 弹窗在预设描述行显示版本号（如 `Deep Fix (v2)`），让过期选择可见。
- **提示词边界升级为编译期不变量**：四份提示词字面量由 `static_assert` 固化——任一份漏掉边界声明，或 `kSystemPrompt` 与 `Basic Fix` 发生漂移，**直接编译失败**。
