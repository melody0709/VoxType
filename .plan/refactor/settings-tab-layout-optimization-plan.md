...
> **状态**：方案冻结（几何/接口/守卫三处已对齐源码实测，可进入 M0→M1）  
> **修订**：v2 — 见文末 §7 修订记录  
...
| **Tab 2: 语音识别引擎** | 主引擎选择 + 备用降级 + **紧凑化动态就地配置** | 76 | 434 | 358px | 616px | **+182px** | 最高项 Qwen 仅 434px，Local 含标点仅 314px |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Row 0: 顶部导航**<br>*（由 `TabSpeechEngine` 创建一次，**不属于本面板**）* | ASR Backend Label / Combo | 42 / 188 | 76 | 130 / 250 | 30 / 150 | "ASR Backend"（主识别引擎） |
| | Fallback Label / Combo | 460 / 530 | 76 | 60 / 210 | 30 / 150 | "Fallback"（备用降级引擎）；与现状 `FallbackLabelX = 472` / `FallbackComboX = 548` 相差 12–18px，落地时**统一以 `ui_types.h` 常量为准并同步修正本表** |
...
| *Hint 1* | **单行精炼语种说明 (Static)** | **188** | **288** | **562** | **24** | `Blank hints fall back to the selected language.`（45 字符）<br>*(消除原本 2 段共 96px 双行大文本，立省 72px；`H` 取 `QwenHintH = 24`，**不可再压到 20**——见 §6.5 的字高下限推导)* |
...
| *Hint 2* | **单行精炼上下文说明 (Static)** | **188** | **344** | **562** | **24** | `200 ms recommended; context sends up to 400 chars.`（49 字符）<br>*(消除原本 48px 双行说明，立省 24px；**文案硬上限约 55 字符**——562px 宽 ÷ 144 DPI 下 9pt Segoe UI 的 ≈9px 字符宽，超出必须回退 `QwenHint2LineH = 48` 两行版)* |
...
| *Hint 3* | **单行精炼高级说明 (Static)** | **188** | **410** | **562** | **24** | `Hotwords, context, sensitive words, punctuation, VAD.`（51 字符）<br>*(单行紧凑呈现；`Y` 由 414 上移到 **410**，使底部仍精确落在 434)* |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Row 0: 顶部导航**<br>*（由 `TabSpeechEngine` 创建一次，**不属于本面板**，宽度沿用 §3.2.2）* | ASR Backend / Fallback | 42 / 460 | 76 | 统一控件 | 30 / 150 | 全局统一主/备引擎下拉框 |
...
| *Hint* | **单行精炼说明 (Static)** | **188** | **340** | **562** | **24** | `Hotwords, correction tables, DDC, end-window, JSON.`（52 字符）<br>*(`Y` 由 344 上移到 **340**，底部仍精确落在 364；`H` 取 `QwenHintH = 24`)* |
...
| 面板 | 现状内容末行底<br>（不含挂在 Row 0 的 Test） | `[Test Connection]` 归位位置 | 归位后底部 | 余量 (616) |
| :--- | :--- | :--- | :--- | :--- |
